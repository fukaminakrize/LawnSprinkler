...
function scheduleCronJobForJob(job) {
	...
	// Job without any days will crash it, so a litle workaround is needed
...
function cronJobCallback(jobId) {
	...
	var phaseStopCallback = function(port) { return function(nextCallback) {
		...
		port.setState(false);
		port.save(function() {
			// If job is canceled, dont continue to next phase, but return "error" to stop job execution
	...
	Job.findById(jobId).populate("phases.port").exec(function (err, job) { 
		...
		job.phases.forEach(function (phase) {
			// Ensure that phase have some duration
			if (phase.duration <= 0)
				return;
